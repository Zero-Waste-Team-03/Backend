...
export class AddVerificationColumns1779374080195 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "reservations" DROP CONSTRAINT "CHK_reservations_quantity_positive"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" RENAME COLUMN "isMailVerified" TO "isVerified"`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "approvedAt" TIMESTAMP`,
    );
    await queryRunner.query(`ALTER TABLE "donations" ADD "approvedById" uuid`);
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "rejectedAt" TIMESTAMP`,
    );
    await queryRunner.query(`ALTER TABLE "donations" ADD "rejectedById" uuid`);
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "rejectionReason" text`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" ALTER COLUMN "status" SET DEFAULT 'Draft'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."donations_status_enum_old" AS ENUM('Draft', 'Published', 'Reserved', 'Completed', 'Expired')`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ALTER COLUMN "status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "rejectionReason"`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "rejectedById"`,
    );
    await queryRunner.query(`ALTER TABLE "donations" DROP COLUMN "rejectedAt"`);
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "approvedById"`,
    );
    await queryRunner.query(`ALTER TABLE "donations" DROP COLUMN "approvedAt"`);
    await queryRunner.query(
      `ALTER TABLE "users" RENAME COLUMN "isVerified" TO "isMailVerified"`,
    );
    await queryRunner.query(
