...
export class MergeAdvertisementIntoDonations1775202984428 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "attachmentId"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."donations_urgency_enum" AS ENUM('Low', 'Medium', 'High')`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "urgency" "public"."donations_urgency_enum" NOT NULL DEFAULT 'Medium'`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "safetyChecklistCompleted" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(`ALTER TABLE "donations" ADD "locationId" uuid`);
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "publishedAt" TIMESTAMP`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "listingExpiresAt" TIMESTAMP`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "listingExpiresAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "publishedAt"`,
    );
    await queryRunner.query(`ALTER TABLE "donations" DROP COLUMN "locationId"`);
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "safetyChecklistCompleted"`,
    );
