...
export class AddWeightDonation1775913832789 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" ADD "foodWeightKg" double precision`,
    );
    await queryRunner.query(
      `UPDATE "donations" SET "foodWeightKg" = "quantity"::double precision * 0.5 WHERE "foodWeightKg" IS NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "donations" ALTER COLUMN "foodWeightKg" SET NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "donations" DROP COLUMN "foodWeightKg"`,
    );
